...
section: tools_1
source: https://support.mitgo.com/knowledge-base/article/how-to-add-banners-to-your-affiliate-program_10
---
...
Banners are graphic images of advertising nature used by publishers for promoting your goods and services.


To make publishers' work more effective, we recommend adding at least 10 banners of various sizes for various product categories.
...
- [Requirements for JPG, PNG, and GIF banners](#img-requirements)
- [HTML5 banner requirements](#html5-requirements)

### General requirements

- To launch a program in Admitad, you must have generally themed banners, which contain information about the program, regardless of seasons, brands, prices, or deals. They'll be used to replace the expired banners.
...
These sizes are most popular among publishers and most clickable among users.
...
- Brands and product groups they advertise*:* Shoes, Apparel, Accessories.
...
Publishers also need these categories. They use them to find relevant banners on your program's page. Make sure your categories have clear names and distribute banners between them accurately.
...
2\.1\. Enter the category name. Remember that publishers will see it.


2\.2\. Flag Activity to activate the created category and make it visible and available to publishers.
...
It's highly advisable that the spare category contains banners of the same size as the main category. In this case, when the main category expires, and its banners are hidden, the same\-size banners from the spare category will replace them. Traffic will keep on coming. But if the spare category doesn't have a banner of the relevant size, the banner's place in the publisher's ad space will become empty.
...
3\.3\. Click Save or Save and start uploading if you want to add banners to the newly created category. Read more about it in [Adding banners](#add-banners).
...
- Deactivate it.  
Deactivation is similar to scheduled deletion. When deactivated, the category and its banners are hidden from publishers after the specified number of days. But they keep generating traffic for about a week, after which the category and its banners are deleted permanently. [How to deactivate a category](#delete-category)
- Delete it.  
The category and its banners will be deleted after the specified period. [How to delete a category](#delete-category)
...
According to the Admitad rules, publishers should be informed of the changes no later than 5 days before such changes. They need this time to remove ad creatives from their ad spaces.
...
1\. No later than 5 days before the deletion/deactivation date, inform your manager of deactivating or deleting a category. The manager will then send newsletters to publishers.


2\. After the manager tells you they have informed the publishers, do the following:
...
If the category status was changed to one of these, you can't change the category or cancel  
...
2\.1\. To do that, in the Link field, paste the address of the page where the user should be taken. For instance, *http://website.com/catalog/women\-shoes/loafers/*

  
2\.2\. After that, add the parameters from the Tracking link for the website field (Program → General settings).


Example: *?utm\_medium\=cpa\&utm\_source\=admitad\&utm\_campaign\={{publisher\_id}\&utm\_content\={{publisher\_id}}\&admitad\_uid\={{admitad\_uid}}*


2\.3\. The final link will look as follows:

*http://website.com/catalog/women\-shoes/loafers/?utm\_medium\=cpa\&utm\_source\=admitad\&utm\_campaign\={{publisher\_id}\&utm\_content\={{publisher\_id}}\&admitad\_uid\={{admitad\_uid}}*
...
> *http://www.yoursite.com/?hl\=en\&admitad\_uid\={{admitad\_uid}}*


3\. In Access rights, you can select who will be able to access the uploaded creative: participants of specific channels or groups (if you have some) or specific publishers.

  
You can also specify the regions for which this ad creative will be created. This may come in handy if you upload banners in different languages or deals only active for specific regions. Using this filter, publishers will find the banners in the relevant language or with an active deal.

  
By default, a banner is for all publishers and all regions.
...
5\. Click Save, and your banner will be uploaded with the Active status—i.e., it'll be visible to publishers on the program page.
...
- Restrict or extend access rights for publishers
...
1\. No later than 5 days before the deletion/deactivation date, create a ticket in which you inform your manager of deactivating or deleting a banner. The manager will then send newsletters to publishers.  
...
- All banners must meet the [requirements](#banner-requirements).
- For each banner, the following must be specified:
	...
	- Group, channel, publisher username (if the banner was created for specific publishers).

## Banner reports


Reports will help you monitor banner performance. [Learn more](https://admitad.useresponse.com/knowledge-base/article/how-to-view-admitad-affiliate-program-reports_6#banner-report)
